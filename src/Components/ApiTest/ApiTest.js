...
const ApiTest = () => {
    ...
    return (
        <div>
            <div className='top-head'>API TESTING </div>
            <div className='searchbar'>
                <input type='search' onChange={handleChange} placeholder="You Can Search Here . . . . ." className='set-search form-control' />
            </div>
            <div className='list-items'>
                {load ? (
                    ...
                    filterdata.map((item, index) => (
                        <ol key={index} className='list-margin'>{index + 1}. {item.title}</ol>
                        ))
                    )
                    }
            </div>
